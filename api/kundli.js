...
export default async function handler(request, response) {
  ...
  let input;
  try { input = typeof request.body === "string" ? JSON.parse(request.body) : request.body || {}; } catch { return sendJson(response, { error: "Invalid request body." }, 400); }
  const birth = validInput(input);
  if (!birth) return sendJson(response, { error: input.language === "hi" ? "कृपया जन्म स्थान, जन्म तिथि, समय और समय क्षेत्र सही भरें।" : "Enter a valid birth place, date, time and time zone." }, 400);
  const datetime = `${input.date}T${input.time}:00${input.timezone}`;
  try {
    ...
    const [basic, chandra, d9] = await Promise.all([navamsha("kundali/basic", navamshaInput), navamsha("kundali/chandra-chart", navamshaInput), navamsha("divisional/d9", navamshaInput)]);
